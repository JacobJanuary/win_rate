...
Version: 6.0 - Конфигурируемое окно анализа с 5-минутными свечами
Обрабатывает только последние 31 день
...
1. Заменить fas.market_data_aggregated на fas_v2.market_data_aggregated в методах:
   ...
   - analyze_signal_both_directions() (строка ~551)
...
class ImprovedScoringAnalyzer:
    ...
    def connect(self):
        ...
        try:
            ...
            # Создаем таблицу результатов если её еще нет
    ...
    def get_unprocessed_signals(self, batch_size: int = 10000) -> List[Dict]:
        """Получение пакета необработанных сигналов старше 1 часа и не старше 31 дня"""
        query = """
            SELECT
    ...
    def get_entry_price(self, trading_pair_id: int, signal_time: datetime,
        ...
        # ⚠️ ВРЕМЕННО: Используем старую схему fas вместо fas_v2
        ...
        try:
            ...
            history_query = f"""
                ...
                WHERE trading_pair_id = %s
                    AND timeframe = '5m'
                    AND timestamp >= %s
                    AND timestamp <= %s + INTERVAL '{self.config.analysis_hours} hours'
                ORDER BY timestamp ASC
            """
            ...
            base_data = {
                ...
                'position_size': self.config.position_size,
                'leverage': self.config.leverage,
                'analysis_end_time': actual_entry_time + timedelta(hours=self.config.analysis_hours)
            }
    ...
    def run(self):
        ...
        logger.info("🚀 Начало анализа исторических данных скоринга (v6.0)")
